//好友列表数据
var friendsList = ['贝贝', '静静', '张三', '圆球球','平平'];
...
//添加表情
function addFace() {
    var $faceDiv = $("<div class='faceDiv'></div>");  //记得去css里面设置一下faceDiv属性
    for (var key in userFaces) {
        //拼接字符串  " <img src="faces/1.gif" alt="撇嘴" > "
       var $img = $("<img src='faces/" + key + "' alt='" + userFaces[key] + "' >");
       $faceDiv.append($img); //
    }
      
    $(".insertFace").append($faceDiv).hover(function(){  //把图和表情两个字放到一个框内，这样hover才是整体,悬浮在字体或者框上都会露出表情框
        $faceDiv.show();
    },function(){
        $faceDiv.hide();
    });
    $faceDiv.hide();  //隐藏表情框,顺序就得放在这里，不能放在上面之前
    //把选中的表情放进文本框中，绑定click事件和img标签
    $faceDiv.on('click', 'img', function(){
        var str = $(this).attr("alt");
        $("#msgTxt").val($("#msgTxt").val() + "[" + str + "]");  //不能用$().text;监测字数的时候用的也是val;
    });
}
//@朋友
function atSomeone() {
    ...
    for (var i = 0; i < friendsList.length; i++) {
        var $span = $("<span>@"+friendsList[i]+"</span>");
        $friendDiv.append($span);
    } 
    $(".atSome").append($friendDiv).hover(function(){
        $(".friendDiv").show();
    },function(){
        $(".friendDiv").hide();
    });
    $(".friendDiv").hide();
    $(".atSome").on("click","span",function(){
        var str = $(this).text();
        $("#msgTxt").val($("#msgTxt").val() + str);
    });
}
//发话题
function newTopic () {
...
function txtChange() {
    ...
    function checkMsgTxt() {
        ...
        var str = $msgTxt.val();
        if (str.length > 140) {
            $(".countTxt").html("超过字数：<em>" + (str.length - 140) + "</em>字数");  //得用html，因为是中间的字变化
        } else {
            $(".countTxt").html("还能输入<em>" + (140-str.length) + "</em>字数");
        }
    }
}